...
public:    
    ...
    {
        funcinfo = f;
        asmvar = this->name+"retval";
    }
    FunctionInformation* getFuncInfo()
    {
        return funcinfo;
    }
    void setAsmvar(string s)
    {
        asmvar = s;
    }
    ~SymbolInfo()
...
ScopeTable(int n, ScopeTable* parent) //constructor for all the other scopes
{
    ...
    ID=parentscope->getID()+"_"+to_string(parentscope->getChildCount());
    for(int i=0;i<SIZE;i++) linkedlist.push_back(NULL);
...
void setAsmvar(string str,string asmvar)
{
    int idx=h(str); //print(idx);
    int ChainPos=0;
